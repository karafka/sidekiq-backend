...
RSpec.describe Karafka::Consumers::Includer do
  ...
  describe 'inline with batch processing' do
    ...
    it { expect(consumer_class.include?(backends_scope::Sidekiq)).to eq false }
  ...
  describe 'sidekiq with responder' do
    ...
    it { expect(consumer_class.include?(backends_scope::Inline)).to eq false }
    it { expect(consumer_class.include?(backends_scope::Sidekiq)).to eq true }
    it { expect(consumer_class.include?(features_scope::SingleParams)).to eq true }
    it { expect(consumer_class.include?(features_scope::Responders)).to eq true }
  end
end
